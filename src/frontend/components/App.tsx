...
class IModelComponents extends React.PureComponent<IModelComponentsProps, IModelComponentsState> {
    ...
    public componentDidMount() {
        IModelApp.viewManager.onViewOpen.addOnce(async (vp: Viewport) => {
            ...
            // I ended up commenting out everything lol

            // this._loadElements(this.props.imodel).then((elements: ElementProps[]) => {
                // this._elements = elements;
                // set feature overrides to alter appearance of elements
                vp.featureOverrideProvider = new SampleFeatureOverrideProvider(/*elements, this.state.depthSlice*/);
            // });

            this._createDecorators();
            this.render();
        });
    }
    ...
    private _createDecorators = () => {
        Data.data.forEach((element: any) => {
            IModelApp.viewManager.addDecorator(new BarDecorator(element.X, element.Y, element.depth));
        });
        // IModelApp.viewManager.addDecorator(new BarDecorator(Data.data));
    };
